...
# In-memory cache mapping MaPhieuMuon -> SoNgayMuon for recent reservations (ephemeral)
reserve_days_cache = {}
...
def reserve_books():
    ...
    # Validate borrowing days: must be integer between 1 and 60
    try:
        so_ngay_muon = int(so_ngay_muon)
    except (ValueError, TypeError):
        return error("Số ngày mượn không hợp lệ", 400)

    if so_ngay_muon < 1 or so_ngay_muon > 60:
        return error("Số ngày mượn phải từ 1 đến 60 ngày", 400)
    ...
    try:
        ...
        # Call stored procedure (SoNgayMuon is not stored at reservation time)
        cursor.execute(
            "EXEC SP_DatTruocSach @MaDocGia=?, @DanhSachJson=?",
            (ma_doc_gia, json_list)
        )
        ...
        # Store requested borrow days in transient cache for UI display
        try:
            if result and len(result) > 0 and result[0] is not None:
                try:
                    ma_phieu = int(result[0])
                    reserve_days_cache[ma_phieu] = so_ngay_muon
                except Exception as _e:
                    print(f"[DEBUG] reserve cache store failed: {_e}")
        except Exception:
            pass
...
def confirm_borrowing(ma_phieu_muon):
    ...
    try:
        ...
        # Remove transient cache entry for this reservation (now confirmed)
        try:
            reserve_days_cache.pop(ma_phieu_muon, None)
        except Exception:
            pass
    ...
    try:
        ...
        for row in rows:
            borrowing_list.append({
                ...
                "trang_thai": row[10],
                # Provide transient so_ngay_muon (fallback to default 14 if unknown)
                "so_ngay_muon": int(reserve_days_cache.get(int(row[0]), 14)),
