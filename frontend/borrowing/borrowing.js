...
// Submit buttons (found from forms) — used to enable/disable when days invalid
const reserveSubmitBtn = reserveForm ? reserveForm.querySelector('button[type="submit"]') : null;
const confirmSubmitBtn = confirmForm ? confirmForm.querySelector('button[type="submit"]') : null;
...
function showMessage(elementId, type, message) {
    ...
    // Ensure it's visible even if CSS rules are not applied yet
...
// ============================================================
// USER CONTEXT FOR BORROWING
// ============================================================
function getBorrowUserContext() {
...
// Validate borrow days inputs and disable submit when > 60
function validateReserveDays() {
    if (!borrowDays) return;
    const val = parseInt(borrowDays.value, 10);
    if (!isNaN(val) && val > 60) {
        showMessage("#reserveMessage", "error", "Bạn chỉ được mượn tối đa 60 ngày!");
        if (reserveSubmitBtn) reserveSubmitBtn.disabled = true;
    } else {
        // only clear the specific max-days error if present
        try {
            const el = document.querySelector('#reserveMessage');
            if (el && el.textContent === 'Bạn chỉ được mượn tối đa 60 ngày!') hideMessage('#reserveMessage');
        } catch (e) {}
...
if (addBookBtn) {
    addBookBtn.addEventListener("click", async (e) => {
        e.preventDefault();
        const ctx = getBorrowUserContext();
        if (!ctx.isEmployee && !ctx.memberId) {
            showMessage("#reserveMessage", "error", "Vui lòng đăng nhập bằng tài khoản độc giả để đặt trước sách.");
            return;
        }
        await addBorrowBook();
    });
...
if (reserveForm) {
    ...
    // Client-side validation: borrow days must be between 1 and 60
    if (isNaN(borrowDaysValue) || borrowDaysValue < 1 || borrowDaysValue > 60) {
        showMessage("#reserveMessage", "error", "Số ngày mượn phải từ 1 đến 60");
        return;
    }
...
async function loadWaitingPickupTickets(memberId = '') {
    ...
    try {
        ...
        if (records.length === 0) {
            waitingTicketsTable.innerHTML = '<tr><td colspan="6" class="empty-state">Không có phiếu chờ lấy</td></tr>';
            return;
        }
        ...
        waitingTicketsTable.innerHTML = records.map(r => `
            <tr>
                ...
                <td>${typeof r.so_ngay_muon !== 'undefined' && r.so_ngay_muon !== null ? r.so_ngay_muon : 14}</td>
...
if (confirmForm) {
    ...
    try {
        ...
        // Reset form (keep staffId to remain filled for the logged-in employee)
...
async function loadBorrowingList(status = "", memberId = "") {
    ...
    const ctx = getBorrowUserContext();
    const effectiveMemberId = memberId || (ctx.isEmployee ? '' : (ctx.memberId || ''));

    if (!ctx.isEmployee && !effectiveMemberId) {
        borrowingListTable.innerHTML = '<tr><td colspan="8" class="empty-state">Không xác định được độc giả đang đăng nhập.</td></tr>';
        showMessage("#listMessage", "error", "Vui lòng đăng nhập bằng tài khoản độc giả để xem danh sách phiếu mượn.");
    ...
    try {
        ...
        if (effectiveMemberId) params.append("member_id", effectiveMemberId);
...
async function loadExpiringBorrowings(memberId = '') {
    ...
    if (!ctx.isEmployee && !effectiveMemberId) {
        expiringTable.innerHTML = '<tr><td colspan="7" class="empty-state">Không xác định được độc giả đang đăng nhập.</td></tr>';
        showMessage('#expiringMessage', 'error', 'Vui lòng đăng nhập bằng tài khoản độc giả để xem thống kê.');
        return;
    }
    ...
    try {
        ...
        if (effectiveMemberId) {
            records = records.filter(r => String(r.ma_doc_gia) === String(effectiveMemberId));
        }
...
async function loadOverdueBorrowings(memberId = '') {
    ...
    if (!ctx.isEmployee && !effectiveMemberId) {
        overdueTable.innerHTML = '<tr><td colspan="7" class="empty-state">Không xác định được độc giả đang đăng nhập.</td></tr>';
        showMessage('#overdueMessage', 'error', 'Vui lòng đăng nhập bằng tài khoản độc giả để xem thống kê.');
        return;
    }
...
async function loadBorrowingHistory(memberId = '', page = 1, perPage = 50) {
    ...
    if (!ctx.isEmployee && !effectiveMemberId) {
        historyTable.innerHTML = '<tr><td colspan="8" class="empty-state">Không xác định được độc giả đang đăng nhập.</td></tr>';
        showMessage('#historyMessage', 'error', 'Vui lòng đăng nhập bằng tài khoản độc giả để xem lịch sử.');
        return;
    }
    ...
    try {
        let result;
        if (effectiveMemberId) {
            result = await apiRequest(`/borrowing/member/${effectiveMemberId}/history?page=${page}&per_page=${perPage}`);
        } else {
...
document.addEventListener("DOMContentLoaded", () => {
    try {
        const ctx = getBorrowUserContext();
        const memberIdDefault = ctx.isEmployee ? '' : (ctx.memberId || '');
    ...
    // Attach live validation for borrow days fields
    try {
        if (borrowDays) {
            borrowDays.addEventListener('input', validateReserveDays);
            borrowDays.addEventListener('change', validateReserveDays);
            // run initial validation
            validateReserveDays();
        }
    } catch (e) {
        console.warn('Could not attach reserveDays listeners', e);
    }

    // confirmBorrowDays field removed — no listeners to attach
});
...
function applyUIByRole() {
    try {
        ...
        // Reserve form inputs and buttons
        const readerInput = document.querySelector('#readerId');
        const borrowDaysInput = document.querySelector('#borrowDays');
        const isbnInput = document.querySelector('#isbn');
        const quantityInput = document.querySelector('#quantity');
        const addBtn = document.querySelector('#addBookBtn');
        const reserveSubmitBtn = reserveForm ? reserveForm.querySelector('button[type="submit"]') : null;

        // If logged-in reader: prefill and enable reserve UI
        if (!ctx.isEmployee && ctx.memberId) {
            if (readerInput) { readerInput.value = ctx.memberId; readerInput.disabled = true; }
            if (borrowDaysInput) borrowDaysInput.disabled = false;
        ...
        // If not logged-in reader: disable reserve UI and show login notice
        if (!ctx.isEmployee && !ctx.memberId) {
            if (readerInput) { readerInput.value = ''; readerInput.disabled = true; }
            if (borrowDaysInput) borrowDaysInput.disabled = true;
            if (isbnInput) isbnInput.disabled = true;
        ...
        // If employee: keep reserve UI hidden/disabled
        if (ctx.isEmployee) {
            if (readerInput) { readerInput.value = ''; readerInput.disabled = true; }
            if (borrowDaysInput) borrowDaysInput.disabled = true;
            if (isbnInput) isbnInput.disabled = true;
        ...
        if (historyInput) {
            if (!ctx.isEmployee && authUser && authUser.MaDocGia) {
