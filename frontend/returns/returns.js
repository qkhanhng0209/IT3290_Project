...
function getConditionBadge(condition) {
    if (!condition) return '<span class="badge">-</span>';
    ...
    const display = map[c] || c;
    let badgeClass = '';
    if (display === 'Bình thường') badgeClass = 'badge-normal';
    else if (display === 'Hỏng nhẹ') badgeClass = 'badge-light';
    else if (display === 'Hỏng nặng') badgeClass = 'badge-heavy';
    else if (display === 'Mất sách') badgeClass = 'badge-lost';
...
async function loadReturnList(append = false) {
    ...
    if (!append) {
        listReturnsTable.innerHTML = '<tr><td colspan="8" class="loading">Đang tải dữ liệu...</td></tr>';
    }
    ...
    try {
        ...
        if (return_records.length === 0) {
            html = '<tr><td colspan="8" class="empty-state">Không có dữ liệu</td></tr>';
        } else {
            return_records.forEach(record => {
                const currentCond = record.tinh_trang_hien_tai || record.ten_tinh_trang_tra || record.tinh_trang_tra;
                html += '<tr>' +
                    '<td><strong>' + escapeHtml(record.ma_phieu_muon) + '</strong></td>' +
                    '<td>' + escapeHtml(record.ten_doc_gia) + '</td>' +
                    '<td>' + escapeHtml(record.ma_sach) + '</td>' +
                    '<td>' + escapeHtml(record.ten_sach) + '</td>' +
                    '<td>' + formatDate(record.han_tra) + '</td>' +
                    '<td>' + formatDate(record.ngay_tra) + '</td>' +
                    '<td>' + getConditionBadge(currentCond) + '</td>' +
        ...
        listReturnsTable.innerHTML = '<tr><td colspan="8" class="empty-state">Lỗi: ' + escapeHtml(error.message) + '</td></tr>';
        if (listMessage) {
